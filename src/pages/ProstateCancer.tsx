              ...
              <div>
                ...
                <ul className="space-y-2 text-gray-700">
                  <li className="flex items-start">
                    ...
                    <div>
                      <strong>Riesgo Muy Bajo:</strong> {language === 'es' ? 'PSA menor de 10, Gleason 6, T1c, menos de 3 cilindros positivos' : 'PSA moins de 10, Gleason 6, T1c, moins de 3 cylindres positifs' : 'PSA less than 10, Gleason 6, T1c, fewer than 3 positive cores'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>Riesgo Bajo:</strong> {language === 'es' ? 'PSA menor de 10, Gleason 6, T1-T2a' : 'PSA moins de 10, Gleason 6, T1-T2a' : 'PSA less than 10, Gleason 6, T1-T2a'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>Riesgo Intermedio:</strong> {language === 'es' ? 'PSA 10-20 o Gleason 7 o T2b-T2c' : 'PSA 10-20 ou Gleason 7 ou T2b-T2c' : 'PSA 10-20 or Gleason 7 or T2b-T2c'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>Riesgo Alto:</strong> {language === 'es' ? 'PSA mayor de 20 o Gleason 8-10 o T3a' : 'PSA supérieur à 20 ou Gleason 8-10 ou T3a' : 'PSA over 20 or Gleason 8-10 or T3a'}
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  <li>• <strong>{language === 'es' ? 'Edad:' : 'Âge:' : 'Age:'}</strong> {language === 'es' ? 'Mayor riesgo después de los 50 años. 80% de casos en mayores de 65 años' : 'Risque accru après 50 ans. 80% des cas chez les plus de 65 ans' : 'Increased risk after 50 years. 80% of cases in those over 65'}</li>
                  <li>• <strong>{language === 'es' ? 'Etnia:' : 'Origine ethnique:' : 'Race/Ethnicity:'}</strong> {language === 'es' ? 'Hombres afroamericanos tienen mayor riesgo (60% más alto)' : 'Hommes afro-américains ont risque plus élevé (60% plus haut)' : 'African American men have higher risk (60% higher)'}</li>
                  <li>• <strong>{language === 'es' ? 'Historia familiar:' : 'Histoire familiale:' : 'Family history:'}</strong> {language === 'es' ? 'Riesgo duplicado si padre/hermano afectado. Triple si múltiples familiares' : 'Risque doublé si père/frère affecté. Triple si multiples membres famille' : 'Risk doubled if father/brother affected. Triple if multiple family members'}</li>
                  <li>• <strong>{language === 'es' ? 'Genética:' : 'Génétique:' : 'Genetics:'}</strong> BRCA2, HOXB13, Lynch syndrome</li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  <li>• <strong>{language === 'es' ? 'Dieta:' : 'Régime:' : 'Diet:'}</strong> {language === 'es' ? 'Dieta alta en grasas saturadas y carnes rojas aumenta riesgo' : 'Régime riche en graisses saturées et viandes rouges augmente risque' : 'Diet high in saturated fats and red meat increases risk'}</li>
                  <li>• <strong>{language === 'es' ? 'Obesidad:' : 'Obésité:' : 'Obesity:'}</strong> {language === 'es' ? 'IMC mayor de 30 asociado con cáncer más agresivo' : 'IMC supérieur à 30 associé avec cancer plus agressif' : 'BMI over 30 associated with more aggressive cancer'}</li>
                  <li>• <strong>{language === 'es' ? 'Actividad física:' : 'Activité physique:' : 'Physical activity:'}</strong> {language === 'es' ? 'Ejercicio regular reduce riesgo en 10-30%' : 'Exercice régulier réduit risque de 10-30%' : 'Regular exercise reduces risk by 10-30%'}</li>
                  <li>• <strong>{language === 'es' ? 'Tabaquismo:' : 'Tabagisme:' : 'Smoking:'}</strong> {language === 'es' ? 'Aumenta riesgo de cáncer agresivo y mortalidad' : 'Augmente risque de cancer agressif et mortalité' : 'Increases risk of aggressive cancer and mortality'}</li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-sm text-gray-700">
                  <li>• <strong>PSA + Tacto rectal:</strong> {language === 'es' ? 'Anual desde los 50 años (45 si alto riesgo)' : 'Annuel dès 50 ans (45 si haut risque)' : 'Annual from age 50 (45 if high risk)'}</li>
                  <li>• <strong>Resonancia multiparamétrica:</strong> {language === 'es' ? 'Si PSA elevado antes de biopsia' : 'Si PSA élevé avant biopsie' : 'If elevated PSA before biopsy'}</li>
                  <li>• <strong>Biopsia transrectal:</strong> {language === 'es' ? '12 cilindros estándar, dirigida por RM si disponible' : '12 cylindres standard, dirigée par IRM si disponible' : '12 core standard, MRI-targeted if available'}</li>
              ...
              <div>
                ...
                <ul className="space-y-3 text-gray-700">
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Vigilancia activa:' : 'Surveillance active:' : 'Active surveillance:'}</strong> {language === 'es' ? 'PSA cada 6 meses, tacto rectal anual, biopsia cada 1-3 años' : 'PSA tous les 6 mois, toucher rectal annuel, biopsie tous les 1-3 ans' : 'PSA every 6 months, digital rectal exam annually, biopsy every 1-3 years'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Prostatectomía radical:' : 'Prostatectomie radicale:' : 'Radical prostatectomy:'}</strong> {language === 'es' ? 'Abierta, laparoscópica o robótica. Preservación nerviosa si factible' : 'Ouverte, laparoscopique ou robotique. Préservation nerveuse si faisable' : 'Open, laparoscopic or robotic. Nerve-sparing if feasible'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Radioterapia externa:' : 'Radiothérapie externe:' : 'External beam radiation:'}</strong> {language === 'es' ? 'IMRT/VMAT 76-80 Gy. Hipofraccionamiento moderado alternativa' : 'RCMI/VMAT 76-80 Gy. Hypofractionnement modéré alternatif' : 'IMRT/VMAT 76-80 Gy. Moderate hypofractionation alternative'}
                    </div>
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-3 text-gray-700">
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Tratamiento multimodal:' : 'Traitement multimodal:' : 'Multimodal treatment:'}</strong> {language === 'es' ? 'RT + terapia hormonal 18-36 meses, o prostatectomía + linfadenectomía' : 'RT + hormonothérapie 18-36 mois, ou prostatectomie + lymphadénectomie' : 'RT + hormone therapy 18-36 months, or prostatectomy + lymphadenectomy'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Terapia hormonal:' : 'Hormonothérapie:' : 'Hormone therapy:'}</strong> {language === 'es' ? 'Agonistas LHRH (leuprolida, goserelina) + antiandrogenos (bicalutamida)' : 'Agonistes LHRH (leuprolide, goséréline) + antiandrogènes (bicalutamide)' : 'LHRH agonists (leuprolide, goserelin) + antiandrogens (bicalutamide)'}
                    </div>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="h-4 w-4 mr-2 text-green-500 mt-0.5 flex-shrink-0" />
                    <div>
                      <strong>{language === 'es' ? 'Seguimiento intensivo:' : 'Suivi intensif:' : 'Intensive follow-up:'}</strong> {language === 'es' ? 'PSA cada 3-6 meses, imagen si progresión bioquímica' : 'PSA tous les 3-6 mois, imagerie si progression biochimique' : 'PSA every 3-6 months, imaging if biochemical progression'}
                    </div>
                  </li>
                </ul>
              </div>
